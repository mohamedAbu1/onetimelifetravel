...
export default function TripIncludes({ trip, lang }) {
  ...
  return <motion.section initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-[1.35rem] border border-[#d1b06a]/35 bg-[#151515] p-5 text-[#f4ead8] shadow-[0_18px_50px_rgba(0,0,0,.2)]">
    <div className="mb-5 flex items-center gap-3 border-b border-white/10 pb-4"><span className="grid h-10 w-10 place-items-center rounded-full bg-[#d1b06a]/15 text-[#d1b06a]"><FaCheckCircle /></span><div><p className="text-[10px] uppercase tracking-[0.25em] text-[#d1b06a]">Included in your journey</p><h2 className="mt-1 font-[Cinzel] text-xl font-semibold">{titles[lang] || titles.en}</h2></div></div>
