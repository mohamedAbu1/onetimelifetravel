...
export default function CalendarWidget({ trip, id }) {
  ...
  return <section className="trip-calendar-widget w-full rounded-[1.35rem] border border-[#d1b06a]/45 bg-[#151515] p-5 text-[#f4ead8] shadow-[0_18px_50px_rgba(0,0,0,.3)]">
...
function GuestCounter({ label, hint, value, onDecrease, onIncrease }) {
  return <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] p-3"><div><p className="text-sm font-semibold text-[#f4ead8]">{label}</p><p className="mt-1 text-[10px] uppercase tracking-wider text-[#918879]">{hint}</p></div><div className="flex items-center gap-2"><button type="button" aria-label={`Decrease ${label}`} onClick={onDecrease} disabled={!value} className="grid h-8 w-8 place-items-center rounded-full border border-white/15 text-[#d1b06a] transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-30">−</button><span className="w-5 text-center text-sm text-[#f4ead8]" aria-live="polite">{value}</span><button type="button" aria-label={`Increase ${label}`} onClick={onIncrease} className="grid h-8 w-8 place-items-center rounded-full bg-[#d1b06a] text-lg text-[#15120e] transition hover:bg-[#ead39e]">+</button></div></div>;
}
