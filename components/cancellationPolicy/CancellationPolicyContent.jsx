...
export default function CancellationPolicyContent() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }} className="policy-content-modern">
      <div className="policy-document-head"><div><p className="policy-kicker">OneTimeLifeTravel · Guest care</p><h2>{t("title")}</h2></div><span className="policy-status">Important</span></div>
      <p className="policy-effective"><strong>{t("effectiveDateLabel", { defaultValue: "Effective date:" })}</strong> {t("effectiveDate")}</p>
      <div className="policy-callout">Please review these terms before confirming your booking. If something is unclear, our team is happy to help.</div>
      ...
      <section className="policy-contact-card"><p className="policy-kicker">Questions about your booking?</p><h3>{t("contact")}</h3><ul><li><strong>{t("ownerLabel")}</strong> {t("owner")}</li><li><strong>{t("emailLabel")}</strong> {t("email")}</li><li><strong>{t("phoneLabel")}</strong> {t("phone")}</li></ul></section>
    </motion.div>
  );
}
