...
export default function CarBookingSection() {
  ...
  return (
    <section className="site-section relative overflow-hidden px-6 lg:px-10" aria-labelledby="car-transfer-title">
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ duration: 0.7 }} className="order-2 lg:order-1">
          <p className="mb-5 flex items-center gap-3 text-xs font-bold uppercase tracking-[0.3em] text-[var(--logo-border)]"><span className="h-px w-10 bg-[var(--logo-border)]" />Signature mobility</p>
          ...
          {user ? <motion.button whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }} onClick={() => window.dispatchEvent(new CustomEvent("openCarBookingChat"))} className="editorial-button dust-interactive mt-9 inline-flex items-center gap-3 bg-[var(--logo-border)] px-7 py-3.5 text-sm text-[#15120e]"><FaCarSide />{t("Book")}</motion.button> : <p className="mt-9 text-sm font-semibold italic text-[var(--sub-text)]">Please log in and book your car transfer.</p>}
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ duration: 0.7 }} className="order-1 lg:order-2">
          <div className="editorial-card relative overflow-hidden rounded-[2rem] border p-3">
            ...
            <div className="relative flex min-h-[360px] flex-col justify-between overflow-hidden rounded-[1.5rem] border border-[var(--logo-border)]/25 px-5 pb-6 pt-5 sm:min-h-[440px] sm:px-8">
              ...
              <div className="relative z-10 flex items-end justify-between gap-4 border-t border-[var(--logo-border)]/30 pt-4"><span className="font-[Cinzel] text-xl text-[var(--heading)]">Travel in comfort</span><span className="text-right text-[10px] uppercase tracking-[0.2em] text-[var(--sub-text)]">Luxor<br />Aswan</span></div>
            </div>
          </div>
        </motion.div>
      </div>
