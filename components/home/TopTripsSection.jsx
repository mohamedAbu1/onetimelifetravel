...
export default function TopTripsSection() {
  ...
  if (loadingTrips) {
    return <section className="site-section w-full px-6 py-20" aria-label={tc("loading")}><div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-2 lg:grid-cols-3">{[1, 2, 3].map((item) => <div key={item} className="h-[430px] animate-pulse rounded-[1.75rem] bg-white/10" />)}</div></section>;
  }
  ...
  if (!topTrips.length) {
    return <section id="featured-trips" className="site-section w-full px-6 py-20 text-center"><p className="text-[10px] font-semibold uppercase tracking-[0.32em] text-[#d1b06a]">{tc("featuredJourneys")}</p><h2 className="mt-4 font-[Cinzel] text-3xl text-[#f4ead8]">{tc("nextStory")}</h2><p className="mx-auto mt-4 max-w-lg text-sm leading-7 text-[#b8b0a2]">{tc("preparingJourneys")}</p></section>;
  }
  ...
  return (
    <section id="featured-trips" className={`site-section w-full overflow-hidden px-5 py-20 md:px-8 lg:px-12 ${theme.background}`}>
      <div className="mx-auto max-w-7xl">
        <div className="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.32em] text-[#d1b06a]">One Time Life Travel · {tc("curatedJourneys")}</p><h2 className="font-[Cinzel] text-3xl font-semibold text-[#f4ead8] md:text-5xl">{t("TopTrips")}</h2></div><p className="max-w-md text-sm leading-7 text-[#b8b0a2] md:text-right">{tc("reviewsCopy")}</p></div>
